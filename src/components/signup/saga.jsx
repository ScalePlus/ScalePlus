...
function* linkedinRegisterSaga(data) {
  yield put({ type: LINKEDIN_SIGNUP_LOADING });
  try {
    let res = yield call(Api.linkedinRegister, data.payload);
    if (res.status) {
      yield put({ type: LINKEDIN_SIGNUP_ERROR, payload: res.message });
    } else {
      yield put({ type: LINKEDIN_SIGNUP_SUCCESS, payload: res.result });
    ...
    yield put({ type: LINKEDIN_SIGNUP_ERROR, payload: error.message });
  }
}
...
function* watchSignupAsync() {
  ...
  yield takeLatest(LINKEDIN_SIGNUP_ACTION, linkedinRegisterSaga);
}
